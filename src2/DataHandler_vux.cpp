...
namespace serialize_some_data
{

    void saveScanData(int scan_id, const Sophus::SO3 &R, const V3D &t, double lidar_end_time,
                      const PointCloudXYZI::Ptr &feats_undistort, const std::string &folder_path)
    {
        std::string txt_file = folder_path + "/scan_" + std::to_string(scan_id) + ".txt";
        std::string pcd_file = folder_path + "/scan_" + std::to_string(scan_id) + ".pcd";

        std::ofstream ofs(txt_file);
        if (ofs.is_open())
        {
            ofs << std::fixed << std::setprecision(9);
            ofs << "timestamp " << lidar_end_time << "\n";
            ofs << "rotation " << R.log().transpose() << "\n"; // Save SO(3) log vector
            ofs << "translation " << t.transpose() << "\n";    // Save translation
            ofs.close();
        }

        pcl::io::savePCDFileBinary(pcd_file, *feats_undistort);
        std::cerr << "saved: " << txt_file << std::endl;
    }

    bool loadScanData(int scan_id, Sophus::SO3 &R, V3D &t, double &lidar_end_time,
                      PointCloudXYZI::Ptr &feats_undistort, const std::string &folder_path)
    {
        std::string txt_file = folder_path + "/scan_" + std::to_string(scan_id) + ".txt";
        std::string pcd_file = folder_path + "/scan_" + std::to_string(scan_id) + ".pcd";

        std::cerr << "load: " << txt_file << std::endl;
        ...
        iss_time >> dummy >> lidar_end_time;

        // Read SO3 log vector
        std::getline(ifs, line);
        std::istringstream iss_rot(line);
        V3D so3_log;
        iss_rot >> dummy >> so3_log(0) >> so3_log(1) >> so3_log(2);
        R = Sophus::SO3::exp(so3_log);

        // Read translation vector
        std::getline(ifs, line);
        std::istringstream iss_trans(line);
        iss_trans >> dummy >> t(0) >> t(1) >> t(2);

        ifs.close();

        // Load point cloud
        feats_undistort.reset(new PointCloudXYZI);
        if (pcl::io::loadPCDFile<PointType>(pcd_file, *feats_undistort) == -1)
            return false;

        return true;
    }

    bool writeSE3ToFile(const std::string &filename, const Sophus::SE3 &transform)
    {
        ...
        file.close();
        std::cout << "Transform saved to " << filename << "\n";
        return true;
    }
...
using namespace serialize_some_data;
...
{
    // if(false) //some debugging
    // {
    //     //do the debug here
    //     // Define initial pose variable
    //     gtsam::Pose3 initialPose = Pose3(Rot3::RzRyRx(0.0, 0.0, 0.0), Point3(0.0, 0.0, 0.0));
    //     gtsam::Values initial;
    //     initial.insert(0, initialPose);

    //     // Add a factor
    //     gtsam::Point3 measured(1, 0, 0);
    //     gtsam::Point3 target(1.5, 2.0, .5);
    //     gtsam::Point3 normal(1.0, 0.0, 0.0);            // x-normal plane
    //     gtsam::SharedNoiseModel model = noiseModel::Isotropic::Sigma(3, 1.);

    //     gtsam::NonlinearFactorGraph graph;
    //     graph.emplace_shared<PointToPointFactor>(0, measured, target, model);

    //     // Create factor
    //     PointToPointFactor factor(0, measured, target, model);

    //     // Evaluate error and Jacobian
    //     //gtsam::Matrix36 H;
    //     Eigen::MatrixXd H(3,6);
    //     gtsam::Vector error = factor.evaluateError(initialPose, &H); // computes H =∂(world_point)/∂pose

    //     std::cout << "\n\nMeasured point    : " << measured.transpose() << std::endl;
    //     std::cout << "Target point      : " << target.transpose() << std::endl;
    //     std::cout << "Pose translation  : " << initialPose.translation().transpose() << std::endl;
    //     std::cout << "Transformed point : " << initialPose.transformFrom(measured).transpose() << std::endl;
    //     std::cout << "Error             : " << error.transpose() << std::endl;
    //     std::cout << "Jacobian (3x6)    : \n" << H << std::endl;
    ...
    //     initialPose = result.at<Pose3>(0);
    //     error = factor.evaluateError(initialPose, &H);
    //     std::cout << " Optimized Error             : " << error.transpose() << std::endl;

    //     //---------------------------------------------------------------------------------------------------------
    //     std::cout<<"\n\n p2plane test..."<<std::endl;
    //     double negative_OA_dot_norm = -target.dot(normal); // -OA.n = -(1.5 * 1) = -1.5
    //     bool use_alternative = true;// false;

    //     graph.resize(0);
    //     initial.clear();

    //     initialPose = Pose3(Rot3::RzRyRx(0.0, 0.0, 0.0), Point3(0.0, 0.0, 0.0));

    //     model = noiseModel::Isotropic::Sigma(1, 3);
    //     PointToPlaneFactor factor_plane(0, measured, normal.normalized(), target, negative_OA_dot_norm, use_alternative, model);

    //     Eigen::MatrixXd H_plane(1,6);  // Jacobian
    //     error = factor_plane.evaluateError(initialPose, &H_plane);

    //     initial.insert(0, initialPose);
    //     graph.emplace_shared<PointToPlaneFactor>(
    //         0, measured, normal.normalized(), target, -target.dot(normal), false,
    //         noiseModel::Isotropic::Sigma(1, 0.01));

    //     // Debug print
    //     std::cout << "Measured point (sensor frame): " << measured.transpose() << "\n";
    //     std::cout << "Target point (on plane)      : " << target.transpose() << "\n";
    //     std::cout << "Plane normal (unit)          : " << normal.normalized().transpose() << "\n";
    //     std::cout << "Initial Pose (Identity)\n";
    //     std::cout << "Transformed point (world)    : " << initialPose.transformFrom(measured).transpose() << "\n";
    //     std::cout << "Error                        : " << error << "\n";
    //     std::cout << "Jacobian (1x6)               : \n" << H_plane << "\n";

    //     gtsam::LevenbergMarquardtOptimizer optimizer_planes(graph, initial);
    //     result = optimizer_planes.optimize();
...
// #endif
    ...
    std::cerr << std::fixed << std::setprecision(10); // Set fixed-point and high precision
    std::cout << std::fixed << std::setprecision(10); // Set fixed-point and high precision
    ...
    Sophus::SE3 prev_mls, curr_mls, prev_gnss_imu, prev_segment_last_pose = Sophus::SE3();
    ...
    int vux_cloud_id = 0, vux_cloud_next_id = 0;
    pcl::VoxelGrid<VUX_PointType> Ref_voxel_filter; //-----------------------------------------------------------------------
...
#define save_vux_clouds
    for (const rosbag::MessageInstance &m : view)
    {
        ...
        if (sync_packages(Measures))
        {
            ...
            perform_mls_registration = false; // true;
...
#define USE_EKF
            ...
            else
            {
                ...
                if (!als_obj->initted_)
                {
                    state_point.offset_R_L_I = Sophus::SO3(Lidar_R_wrt_IMU);
            ...
            else
            {
                ...
                if (do_once) //&& false
                {
                    if (false) // this will load the reprocessor generated map
                    {
                        ...
                        f1 = "/home/eugeniu/x_vux-georeferenced-final/georeferenced/a2/segment_0.las";
                        ...
                        while (reader.ReadNextPoint())
                        {
                            ...
                            V3D cloudPoint = V3D(p.GetX(), p.GetY(), p.GetZ());
                ...
                while (gnss_vux_data[tmp_index].gps_tod <= time_of_day_sec && tmp_index < gnss_vux_data.size())
                {
                    ...
                    if (perform_mls_registration && false) //&& false
                    {
                        ...
                        else
                        {
                            ...
                            // if (lines_buffer.size() > 700) // got enough data for initial ICP
                            {
...

                                {
                                    ...
                                    bool p2plane_with_mesh_point = false; // true;
                                    bool weighted_p2_plane = true;
                                    ...
                                    for (int iter_num = 0; iter_num < max_iterations_; iter_num++)
                                    {
                                        ...
                                        current_cost = scan2map_GN_omp(init_georeferenced_segment, refference_kdtree,
                                                                       ...
                                                                       p2p, p2plane, local_error, threshold_nn_, p2plane_with_mesh_point, weighted_p2_plane);

                                        // current_cost = scan2map_planes_test(init_georeferenced_segment, refference_kdtree,
                                        //                                     reference_localMap_cloud, q, t, T_icp, threshold_nn_);
                    ...
                    if (false) // this will find the extrinsics between the lidar and the gnss poses
                    {
                        ...
                        {
                            std::cout << "Start extrinsic estiamtion press enter..." << std::endl;
                            std::cin.get();
                    ...
                    while (readVUX.next(next_line))
                    {
                        ...
                        if (next_line->empty())
                        {
                            std::cout << "Break - line empty" << std::endl;
                            break;
                        }
                            ...
                            if (latest_approach)
                            {
...

                                        {
                                            ...
                                            for (int iter_num = 0; iter_num < max_iterations_; iter_num++)
                                            {
                                                ...
                                                current_cost = scan2map_GN_omp(init_georeferenced_segment, refference_kdtree,
                                                                               reference_localMap_cloud, q, t, T_icp,
                                                                               false, prev_segment, kdtree_prev_segment,
                                                                               p2p, p2plane, local_error, threshold_nn_, p2plane_with_mesh_point);
                                            ...
                                            std::cout << "Start building the reference graph..." << std::endl;
                                            // buildReferenceGraph(
                                            //     prev_pub,
                                            //     lines_buffer, refined_line_poses_buffer,
                                            //     refference_kdtree, reference_localMap_cloud);
                                            ...
                                            /// coarse_once = false;   keep it here

                                            prev_pose = refined_line_poses_buffer.back(); // last refined pose
                                            optimized_pose = prev_pose;
                                        }
                                        lines_buffer.clear();
                                        refined_line_poses_buffer.clear();
                                        line_poses_buffer.clear();
                                ...
                                else
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        //publish_refined_ppk_gnss(optimized_pose, cloud_time);

                                        if ((pose_pub.getNumSubscribers() != 0 || pose_pub2.getNumSubscribers() != 0) && false)
                                        {
                                            nav_msgs::Odometry pose_msg;
                                    ...
                                    if (pubOptimizedVUX.getNumSubscribers() != 0 && false)
                                    {
                                        feats_down_body->clear();
                                        for (int i = 0; i < downsampled_line->size(); i++) // for each point in the line
                                        {
...
// shift it to the frame of the prev estimated pose
                                        ...
                                        if (coarse_register) //&& coarse_once
...
//---------------------------------------------------------------------
...

                                {
                                    ...
                                    for (const auto &pt : down_cloud->points)
                                    {
                                        if (pt.range < 20)
                                        {
                                            ...
                                            new_pt.y = pt.y;
                                            new_pt.z = pt.z;
                                            feats_undistort->points.push_back(new_pt);
                                        }
                                    }
                                    ...
                                    if (false) // do not compute the errors
                                    {
                                        ...
                                        if (als_ref->Update(Sophus::SE3(state_point.rot, state_point.pos)))
                                        {
                                            als_ref->getCloud(downsampled_als_cloud); // original density
                        ...
                        if (cloud_time > gnss_vux_data[tmp_index].gps_tod)
                        {
                            ...
                            // const auto &last_point_time = next_line->points.back().time;
                            //  std::cout<<"last_point_time:"<<last_point_time<<std::endl;
