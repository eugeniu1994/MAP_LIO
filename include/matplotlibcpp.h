...
template<typename NumericX, typename NumericY>
bool plot(const std::vector<NumericX>& x, const std::vector<NumericY>& y, const std::string& s = "", const std::string& label="")
{
    assert(x.size() == y.size());
...
template<typename Numeric>
void Plot_(const std::vector<Numeric>& y, const std::string& name, const std::string& format = "") {
        detail::_interpreter::get();
        ...
        if(res)
        {
            ...
            if(line)
                PyObject* set_data_fct = PyObject_GetAttrString(line,"set_data");
            else
                Py_DECREF(line);
            Py_DECREF(res);
        }
...
template<typename NumericX, typename NumericY, typename NumericZ, typename NumericU, typename NumericW, typename NumericV, typename StringC>
bool quiver(const std::vector<NumericX>& x, const std::vector<NumericY>& y, const std::vector<NumericZ>& z, 
const std::vector<NumericU>& u, const std::vector<NumericW>& w, const std::vector<NumericV>& v, const std::vector<StringC> color,
const std::map<std::string, std::string>& keywords = {})
{
  ...
  //PyObject* color_array = detail::get_array(color);
  ...
  //PyTuple_SetItem(plot_args, 6, color_array);
...
inline void set_zlimit(Numeric left, Numeric right, const std::map<std::string, std::string>& keywords = {})
{
    ...
    PyObject *zlabel = PyObject_GetAttrString(ax, "set_zlim");
    if (!zlabel) throw std::runtime_error("Attribute set_zlim not found.");
    Py_INCREF(zlabel);

    PyObject *res = PyObject_Call(zlabel, args, kwargs);
    if (!res) throw std::runtime_error("Call to set_zlimit() failed.");
    Py_DECREF(zlabel);
...
class MyClass {      
  public:   
    ...
    bool quiv(const std::vector<NumericX>& x, const std::vector<NumericY>& y, const std::vector<NumericZ>& z, 
             ...
             std::string plot_color = "", const double s=1.0, // The marker size in points**2
             const std::map<std::string, std::string> & keywords = {},
             const long fig_number=0) 
    {
        if(x.size() == 0)
        ...
        PyObject* plot_args = PyTuple_New(6);
        PyObject* kwargs = PyDict_New();
        PyObject *obj_x = nullptr, *obj_y = nullptr,*obj_z = nullptr;
        PyObject *obj_u = nullptr, *obj_v = nullptr,*obj_w = nullptr;

        for(int i=0;i<x.size();i++){
            //PyObject* 
            obj_x = PyFloat_FromDouble(x[i]);
            //PyObject* 
            obj_y = PyFloat_FromDouble(y[i]);
            //PyObject* 
            obj_z = PyFloat_FromDouble(z[i]);

            auto norm = sqrt(u[i]*u[i] + v[i]*v[i] + w[i]*w[i])/15;// / 3.;

            //PyObject* 
            obj_u = PyFloat_FromDouble(u[i]/norm);
            //PyObject* 
            obj_v = PyFloat_FromDouble(v[i]/norm);
            //PyObject* 
            obj_w = PyFloat_FromDouble(w[i]/norm);

            //PyObject* kwargs = PyDict_New();
            kwargs = PyDict_New();
            PyDict_SetItemString(kwargs, "color", PyString_FromString(colors[i%3].c_str()));
            PyDict_SetItemString(kwargs, "length", PyFloat_FromDouble(.01));


            //PyObject* plot_args = PyTuple_New(6);
            plot_args = PyTuple_New(6);
            PyTuple_SetItem(plot_args, 0, obj_x);
            PyTuple_SetItem(plot_args, 1, obj_y);
            PyTuple_SetItem(plot_args, 2, obj_z);

            PyTuple_SetItem(plot_args, 3, obj_u);
            PyTuple_SetItem(plot_args, 4, obj_v);
            PyTuple_SetItem(plot_args, 5, obj_w);

            PyObject *plot3_q = PyObject_GetAttrString(axis, "quiver");
            if (!plot3_q) throw std::runtime_error("No 3D quiver plot");
            Py_INCREF(plot3_q);

            PyObject* res = PyObject_Call(plot3_q, plot_args, kwargs);
            if (!res) {
                throw std::runtime_error("Failed 3D quiver  - "+ std::to_string(i));
            } 
            else{
                Py_DECREF(res);
            }
        ...
        PyObject* list = PyList_New(2);
        PyList_SetItem(list, 0, PyFloat_FromDouble(x[0] - sc));
        PyList_SetItem(list, 1, PyFloat_FromDouble(x[0] + sc));

        PyObject* args_lim = PyTuple_New(1);
        PyTuple_SetItem(args_lim, 0, list);
        
        PyObject *plot3_set_xlim = PyObject_GetAttrString(axis, "set_xlim");
        PyObject *res_ = PyObject_Call(plot3_set_xlim, args_lim, kwargs);
        if (!res_) throw std::runtime_error("Call to set_xlim() failed.");
        Py_DECREF(plot3_set_xlim);

        list = PyList_New(2);
        PyList_SetItem(list, 0, PyFloat_FromDouble(y[0] - sc));
        PyList_SetItem(list, 1, PyFloat_FromDouble(y[0] + sc));
        args_lim = PyTuple_New(1);
        PyTuple_SetItem(args_lim, 0, list);
        PyObject *plot3_set_ylim = PyObject_GetAttrString(axis, "set_ylim");
        res_ = PyObject_Call(plot3_set_ylim, args_lim, kwargs);
        if (!res_) throw std::runtime_error("Call to set_ylim() failed.");
        Py_DECREF(plot3_set_ylim);

        list = PyList_New(2);
        PyList_SetItem(list, 0, PyFloat_FromDouble(z[0] - sc));
        PyList_SetItem(list, 1, PyFloat_FromDouble(z[0] + sc));
        args_lim = PyTuple_New(1);
        PyTuple_SetItem(args_lim, 0, list);
        PyObject *plot3_set_zlim = PyObject_GetAttrString(axis, "set_zlim");
        res_ = PyObject_Call(plot3_set_zlim, args_lim, kwargs);
        if (!res_) throw std::runtime_error("Call to set_zlim() failed.");
        Py_DECREF(plot3_set_zlim);
        Py_DECREF(res_);

        if(plot_color != ""){
            plot3 = PyObject_GetAttrString(axis, "plot3D");
            if (!plot3) throw std::runtime_error("No 3D plot3D plot");
            Py_INCREF(plot3);

            kwargs = PyDict_New();
            PyDict_SetItemString(kwargs, "color", PyString_FromString(plot_color.c_str()));
            res = PyObject_Call(plot3, args, kwargs);
            if (!res) throw std::runtime_error("Failed 3D plot3D plot");
        }
    ...
    bool quiv_vs(
             ...
             const std::vector<NumericX>& x2, const std::vector<NumericY>& y2, const std::vector<NumericZ>& z2, 
             const std::vector<NumericU>& u2, const std::vector<NumericW>& w2, const std::vector<NumericV>& v2,
             ...
             const long fig_number=0) 
    {
        ...
        for(int i=0;i<x.size();i++){
            ...
            auto norm = sqrt(u[i]*u[i] + v[i]*v[i] + w[i]*w[i]) / 3.;
            //auto norm = sqrt(u[i]*u[i] + v[i]*v[i] + w[i]*w[i]) / 10.;
        ...
        for(int i=0;i<x2.size();i++){
            //PyObject* 
            obj_x = PyFloat_FromDouble(x2[i]);
            //PyObject* 
            obj_y = PyFloat_FromDouble(y2[i]);
            //PyObject* 
            obj_z = PyFloat_FromDouble(z2[i]);

            auto norm = sqrt(u2[i]*u2[i] + v2[i]*v2[i] + w2[i]*w2[i]) / 3.;//*300;

            //PyObject* 
            obj_u = PyFloat_FromDouble(u2[i]/norm);
            //PyObject* 
            obj_v = PyFloat_FromDouble(v2[i]/norm);
            //PyObject* 
            obj_w = PyFloat_FromDouble(w2[i]/norm);
        ...
        xarray = detail::get_array(x2);
        yarray = detail::get_array(y2);
        zarray = detail::get_array(z2);
        ...
        plot3 = PyObject_GetAttrString(axis, "scatter");
        if (!plot3) throw std::runtime_error("No 3D scatter plot");
        Py_INCREF(plot3);
        res = PyObject_Call(plot3, args, kwargs);
        if (!res) throw std::runtime_error("Failed 3D scatter plot");
        ...
        PyObject* list = PyList_New(2);
        PyList_SetItem(list, 0, PyFloat_FromDouble(x[middleIndex] - sc));
        PyList_SetItem(list, 1, PyFloat_FromDouble(x[middleIndex] + sc));
        ...
        list = PyList_New(2);
        PyList_SetItem(list, 0, PyFloat_FromDouble(y[middleIndex] - sc));
        PyList_SetItem(list, 1, PyFloat_FromDouble(y[middleIndex] + sc));
        args_lim = PyTuple_New(1);
        PyTuple_SetItem(args_lim, 0, list);
        PyObject *plot3_set_ylim = PyObject_GetAttrString(axis, "set_ylim");
        res_ = PyObject_Call(plot3_set_ylim, args_lim, kwargs);
        ...
        list = PyList_New(2);
        PyList_SetItem(list, 0, PyFloat_FromDouble(z[middleIndex] - sc));
        PyList_SetItem(list, 1, PyFloat_FromDouble(z[middleIndex] + sc));
        args_lim = PyTuple_New(1);
        PyTuple_SetItem(args_lim, 0, list);
        PyObject *plot3_set_zlim = PyObject_GetAttrString(axis, "set_zlim");
        res_ = PyObject_Call(plot3_set_zlim, args_lim, kwargs);
    ...
    bool plot_imu_and_gt(
             ...
             const std::vector<NumericX>& x_GT, const std::vector<NumericY>& y_GT, const std::vector<NumericZ>& z_GT, 
             ...
             const long fig_number=0) 
    {
        ...
        for(int i=0;i<x.size();i++){
            ...
            auto norm = sqrt(u[i]*u[i] + v[i]*v[i] + w[i]*w[i]) * 3.0;
        ...
        //plot the translation as scatter plot
        PyObject *xarray = detail::get_array(x);
        PyObject *yarray = detail::get_array(y);
        PyObject *zarray = detail::get_array(z);
        ...
        Py_DECREF(obj_x); Py_DECREF(obj_y); Py_DECREF(obj_z);
        Py_DECREF(obj_u); Py_DECREF(obj_v); Py_DECREF(obj_w);

        //plot the GT orientation values------------------------------------------------
        for(int i=0;i<x2.size();i++){
            obj_x = PyFloat_FromDouble(x2[i]);
            obj_y = PyFloat_FromDouble(y2[i]);
            obj_z = PyFloat_FromDouble(z2[i]);

            auto norm = sqrt(u2[i]*u2[i] + v2[i]*v2[i] + w2[i]*w2[i]);
        ...
        //orange = start, black = end
        colors = {"orange", "black", "red"};
        for(int i=0; i<x_GT.size(); i ++){
            obj_x = PyFloat_FromDouble(x_GT[i]);
            obj_y = PyFloat_FromDouble(y_GT[i]);
            obj_z = PyFloat_FromDouble(z_GT[i]);

            kwargs = PyDict_New();

            PyDict_SetItemString(kwargs, "color", PyString_FromString(colors[i].c_str()));
            PyDict_SetItemString(kwargs, "s", PyFloat_FromDouble(4*s));
            ...
            PyObject *plot3_q = PyObject_GetAttrString(axis, "scatter");
            if (!plot3_q) throw std::runtime_error("No 3D scatter plot");
            Py_INCREF(plot3_q);

            PyObject* res = PyObject_Call(plot3_q, plot_args, kwargs);
            if (!res) {
                throw std::runtime_error("Failed 3D scatter  - "+ std::to_string(i));
            } 
            else{
                Py_DECREF(res);
            }
    ...
    bool quiv_vs(const std::vector<NumericX>& x, const std::vector<NumericY>& y, const std::vector<NumericZ>& z, 
            ...
            const std::vector<NumericX>& x3, const std::vector<NumericY>& y3, const std::vector<NumericZ>& z3,
             ...
             const long fig_number=0) 
    {
        ...
        for(int i=0;i<x.size();i++){
            ...
            auto norm = sqrt(u[i]*u[i] + v[i]*v[i] + w[i]*w[i]) / 3.;//*300;
        ...
        kwargs = PyDict_New();
        PyDict_SetItemString(kwargs, "s", PyFloat_FromDouble(2*s));
        //PyDict_SetItemString(kwargs, "length", PyFloat_FromDouble(.01));
        ...
        //------------------------------------------------------------------
        xarray = detail::get_array(x3);
        yarray = detail::get_array(y3);
        zarray = detail::get_array(z3);
        ...
        kwargs = PyDict_New();
        PyDict_SetItemString(kwargs, "s", PyFloat_FromDouble(5*s));
        PyDict_SetItemString(kwargs, "color", PyString_FromString("black"));
        
        plot3 = PyObject_GetAttrString(axis, "scatter");
        if (!plot3) throw std::runtime_error("No 3D spline scatter plot");
        Py_INCREF(plot3);
        res = PyObject_Call(plot3, args, kwargs);
        if (!res) throw std::runtime_error("Failed 3D spline scatter plot");
        //------------------------------------------------------------------
        auto sc = .3;
    ...
    template<typename NumericX, typename NumericY, typename NumericZ>
    bool plot3D(const std::vector<NumericX>& x, const std::vector<NumericY>& y, const std::vector<NumericZ>& z, 
             const long fig_number=0) 
    {
        ...
        PyObject* pystring = PyString_FromString("o-");
        PyTuple_SetItem(args, 3, pystring);


        PyObject* kwargs = PyDict_New();

        //PyDict_SetItemString(kwargs, "s", PyFloat_FromDouble(5));
        //PyDict_SetItemString(kwargs, "color", PyString_FromString("green"));

        PyObject *plot3 = PyObject_GetAttrString(axis, "plot3D");
        if (!plot3) throw std::runtime_error("No 3D plot3D plot");
        Py_INCREF(plot3);
        PyObject *res = PyObject_Call(plot3, args, kwargs);
        if (!res) throw std::runtime_error("Failed 3D plot3D plot");
